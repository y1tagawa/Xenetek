...
extension NodeHelper on Node {
  ...
  Node bendNeck({double? radians, double? degrees}) {
    ...
    return _rotate(Doll.neck, Vector3.unitX, radians_)._rotate(Doll.head, Vector3.unitX, radians_);
  }

  // 右胸鎖関節
  //todo: テスト、それとIK的な腕の全関節の動き関数
  Node bendRSc({double? radians, double? degrees}) =>
      _rotate(Doll.rSc, Vector3.unitY, _toRadians(radians, degrees));
  Node twistRSc({double? radians, double? degrees}) =>
      _rotate(Doll.rSc, -Vector3.unitZ, _toRadians(radians, degrees));

  // 右肩
  Node bendRShoulder({double? radians, double? degrees}) =>
      _rotate(Doll.rShoulder, -Vector3.unitZ, _toRadians(radians, degrees));
  Node twistRShoulder({double? radians, double? degrees}) =>
      _rotate(Doll.rShoulder, Vector3.unitX, _toRadians(radians, degrees));
  Node swingRShoulder({double? radians, double? degrees}) =>
      _rotate(Doll.rShoulder, Vector3.unitY, _toRadians(radians, degrees));

  // 左肩
  Node bendLShoulder({double? radians, double? degrees}) =>
      _rotate(Doll.lShoulder, Vector3.unitZ, _toRadians(radians, degrees));

  // 右肘
  Node bendRElbow({double? radians, double? degrees}) =>
      _rotate(Doll.rElbow, Vector3.unitY, _toRadians(radians, degrees));

  // 左肘
  Node bendLElbow({double? radians, double? degrees}) =>
      _rotate(Doll.lElbow, -Vector3.unitY, _toRadians(radians, degrees));
  // todo: 手首
